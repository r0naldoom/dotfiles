...
SmartPanel {
  ...
  panelContent: Rectangle {
    ...
    Item {
      ...
      width: timerText.width + Style.marginXL * 2 + (PowerlineConfig.enabled ? PowerlineConfig.sizeLarge * 2 : 0)
      height: timerText.height + Style.marginL * 2
      z: 1000

      NPowerlineWrapper {
        anchors.fill: parent
        fillColor: Qt.alpha(Color.mSurface, Settings.data.ui.panelBackgroundOpacity)
  ...
  component PowerButton: Item {
    ...
    NPowerlineWrapper {
      ...
      triangleSize: PowerlineConfig.sizeLarge
      border.width: buttonRoot.pending ? Math.max(Style.borderM) : 0
      border.color: buttonRoot.pending ? Color.mPrimary : Color.mOutline
    }
